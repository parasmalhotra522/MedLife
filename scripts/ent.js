...
fetch("./data.json")
    ...
    .then((data) => returnEntMedicines([...data]));

    let entMedicines;
// method which will filter out the medicines specifically for ENT and display it to user
function returnEntMedicines(allMedicines) {
   entMedicines = allMedicines.filter((item) => item.category === "ENT");
   showMedicines(entMedicines);
}
...
function decrementQuantity(index) {
    if(activeUser) {
       ...
       if(entMedicines[index].netQuantity > 0) {
        entMedicines[index].netQuantity -= 1;
        document.getElementById(entMedicines[index].productId).value=(entMedicines[index].netQuantity);
    }
} else {
...
function addToCart(index) {
    if(activeUser) {
        console.log("item to be added to cart", entMedicines[index]);
        cart.push(entMedicines[index]);
        setToUserCart(cart);
    } 
    else {
        alert("Please login before you can buy anything!!!");
