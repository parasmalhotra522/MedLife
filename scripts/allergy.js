...
// ----------- making and filtering the medicines for ENT -----

fetch("./data.json")
    .then((response) => response.json())
    .then((data) => returnallergyMedicines([...data]));

    let allergyMedicines;
// method which will filter out the medicines specifically for ENT and display it to user
function returnallergyMedicines(allMedicines) {
   allergyMedicines = allMedicines.filter((item) => item.category === "ALLERGIES");
   showMedicines(allergyMedicines);
}
...
function decrementQuantity(index) {
    if(activeUser) {
       ...
       if(allergyMedicines[index].netQuantity > 0) {
        allergyMedicines[index].netQuantity -= 1;
        document.getElementById(allergyMedicines[index].productId).value=(allergyMedicines[index].netQuantity);
    }
} else {
...
function addToCart(index) {
    if(activeUser) {
        console.log("item to be added to cart", allergyMedicines[index]);
        cart.push(allergyMedicines[index]);
        setToUserCart(cart);
    } 
    else {
        alert("Please login before you can buy anything!!!");
